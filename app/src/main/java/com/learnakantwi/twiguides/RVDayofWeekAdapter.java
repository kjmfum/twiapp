...
public class RVDayofWeekAdapter extends RecyclerView.Adapter<RVDayofWeekAdapter.MyViewHolder> implements Filterable{
    ...
    private Filter RecycleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<DaysOfWeek> filteredList = new ArrayList<>();
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (DaysOfWeek x : tempArray) {

                    if (x.getNameTwi().toLowerCase().contains(filterPattern) || x.getNameEnglish().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVDayofWeekAdapter(Context context, ArrayList<DaysOfWeek> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getNameEnglish());
        holder.tvTwi.setText(originalArray.get(position).getNameTwi());
    }
