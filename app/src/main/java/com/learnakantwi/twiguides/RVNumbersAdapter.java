...
public class RVNumbersAdapter extends RecyclerView.Adapter<RVNumbersAdapter.MyViewHolder> implements Filterable{
    ...
    private Filter RecycleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<Numbers> filteredList = new ArrayList<>();
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Numbers x : tempArray) {

                    if (x.figure.toLowerCase().contains(filterPattern) || x.getNumberWord().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVNumbersAdapter(Context context, ArrayList<Numbers> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getFigure());
        holder.tvTwi.setText(originalArray.get(position).getNumberWord());
    }
