...
public class RVColoursAdapter extends RecyclerView.Adapter<RVColoursAdapter.MyViewHolder> implements Filterable{



    Context context;
    ArrayList<Colours> originalArray , tempArray;
    LayoutInflater inflater;
    onClickRecycle onClickRecycle;
    private Filter RecycleFilter = new Filter() {
        @Override
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<Colours> filteredList = new ArrayList<>();
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Colours x : tempArray) {

                    if (x.getTwiColours().toLowerCase().contains(filterPattern) || x.getEnglishColours().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVColoursAdapter(Context context, ArrayList<Colours> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getEnglishColours());
        holder.tvTwi.setText(originalArray.get(position).getTwiColours());
    }
