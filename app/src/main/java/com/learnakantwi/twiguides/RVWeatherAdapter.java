...
public class RVWeatherAdapter extends RecyclerView.Adapter<RVWeatherAdapter.MyViewHolder> implements Filterable{



    Context context;
    ArrayList<Weather> originalArray , tempArray;
    ...
    private Filter RecycleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Weather x : tempArray) {

                    if (x.getWeatherTwi().toLowerCase().contains(filterPattern) || x.getWeatherEnglish().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVWeatherAdapter(Context context, ArrayList<Weather> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getWeatherEnglish());
        holder.tvTwi.setText(originalArray.get(position).getWeatherTwi());
    }
