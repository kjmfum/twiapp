...
public class RVTimeAdapter extends RecyclerView.Adapter<RVTimeAdapter.MyViewHolder> implements Filterable{



    Context context;
    ArrayList<Time> originalArray , tempArray;
    LayoutInflater inflater;
    onClickRecycle onClickRecycle;
    private Filter RecycleFilter = new Filter() {
        @Override
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<Time> filteredList = new ArrayList<>();
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Time x : tempArray) {

                    if (x.getTwiTime().toLowerCase().contains(filterPattern) || x.getEnglishTime().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVTimeAdapter(Context context, ArrayList<Time> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getEnglishTime());
        holder.tvTwi.setText(originalArray.get(position).getTwiTime());
    }
