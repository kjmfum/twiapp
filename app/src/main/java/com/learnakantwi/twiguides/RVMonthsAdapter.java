...
public class RVMonthsAdapter extends RecyclerView.Adapter<RVMonthsAdapter.MyViewHolder> implements Filterable{
    ...
    private Filter RecycleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<Months> filteredList = new ArrayList<>();
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Months x : tempArray) {

                    if (x.getTwiMonths().toLowerCase().contains(filterPattern) || x.getEnglishMonths().toLowerCase().contains(filterPattern)) {
                        filteredList.add(x);
                    }
                }
            }
    ...
    public RVMonthsAdapter(Context context, ArrayList<Months> originalArray , onClickRecycle onClickRecycle) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            holder.tvEnglish.setText(originalArray.get(position).getEnglishMonths());
        holder.tvTwi.setText(originalArray.get(position).getTwiMonths());
    }
